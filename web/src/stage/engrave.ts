...
export type EngraveKind = "plain" | "bill" | "page" | "tape" | "accent";
...
export const SPECS: Record<string, EngraveSpec> = {
  ...
  Wood: { tone: 0.6 },
  Ink: { tone: 0.27, shine: 0.5 },
  Chip: { tone: 0.46 },
  Strap: { tone: 0.95, kind: "accent" },
  Ember: { tone: 1.0, kind: "accent" },
};
...
const KIND_ID: Record<EngraveKind, number> = { plain: 0, bill: 1, page: 2, tape: 3, accent: 4 };
...
float engLines(float c, float cover) {
  ...
  float lf = log2(1.0 / (px * uPitch));         // log2 of the wanted lines per unit
...
const FRAG_MAIN = /* glsl */ `
{
  ...
  float spec = pow(max(dot(N, normalize(uLightDir + V)), 0.0), 28.0) * uShine;
