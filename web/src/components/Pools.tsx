...
import { FLAG_GLOSS } from "./PoolsHead";
...
const COLS: Col[] = [
  ...
  { key: "tvlUsd", label: "Money in pool", title: "what the pool holds right now, valued from chain", render: (p) => fmtUsd(p.tvlUsd) },
  ...
  { key: "fees24hUsd", label: "Fees 24h", title: "fees paid to the people whose money is in the pool, last 24h. * measured from on-chain fee counters; otherwise volume × base fee", render: (p) => <>{fmtUsd(p.fees24hUsd)}{p.feesSource === "onchain" ? <span className="onchain" title={`measured on-chain over a ${p.feesWindowHours}h window`}>*</span> : null}</> },
  ...
  { key: "binRangePct", label: "Range", title: "how far the price walked during the sample window, in percent", render: (p) => (p.binRangePct === null ? "n/a" : `${p.binRangePct.toFixed(1)}%`) },
...
export function Pools({ screen, status, now }: PoolsProps) {
  ...
  }, [screen, status, quote, q, minLiq, hideNew, sortKey, dir]);
  ...
  return (
    <section className="pools" id="pools" aria-label="Every pool, ranked">
      <div className="pools-filters">
        ...
        <span className="fld-note">Mr Bands trades SOL-paired pools only, for now</span>
        <label className="fld">
      ...
      <div className="pools-head">
        <span>
          <b>{rows.length}</b> pools · scanned <b>{screen.scannedPools.toLocaleString()}</b> pools on-chain, <b>{screen.livePools.toLocaleString()}</b> traded in the last day
        </span>
        ...
        <table className="pools-table">
          ...
          <tbody>
            {view.map((p) => {
              ...
              return (
                <tr key={p.address} className={st ? `has-status ${st === "in band" ? "is-inband" : "is-watching"}` : undefined}>
                  ...
                  <td className="col-pool">
                    <div className="pool-name">
                      ...
                      <span className="chip tiny" title="bin step: how wide each price step is">{p.binStep} bps</span>
            ...
            {view.length === 0 && (
              <tr><td colSpan={cols.length + 3} className="empty">Nothing matches these filters.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <p className="fine">Money in pool, fees, bin step and fee tiers are read from the pool accounts on Solana. Volume, prices, market cap and pool age come from GeckoTerminal. Fees marked * are measured from on-chain fee counters over the sample window and scaled to 24h; the rest are volume × base fee.</p>
    </section>
  );
}
