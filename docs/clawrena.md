...
and payout are fixed for good, so this is decided once and not reopened.
...
- **Who pays is who gets the creator fees.** ClawPump's docs call `walletAddress` "the Solana base58
  ...
  costs about 0.02-0.05 SOL. The spec in `ops/live.env` under "the Clawrena token" (NVDAx, 300 bps, 2.5 SOL)
  is the old one; another builder owns that section and is bringing it to this spec.
