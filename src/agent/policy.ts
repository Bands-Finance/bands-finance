...
export function policyDecide(o: Observation, x: PolicyExtras): PolicyResult {
  ...
  if (earn && env.maxPaybackHours > 0 && earn.paybackHours !== null && earn.paybackHours > env.maxPaybackHours) {
    return hold(
      `No band in ${o.poolLabel} (${priceLine}). Opening costs about $${r(earn.costUsd, 2)} in rent that does not come back and swap fees${earn.transferFeeUsd > 0 ? `, $${r(earn.transferFeeUsd, 2)} of it the ${r((s.baseToken.transferFee?.bps ?? 0) / 100, 2)}% transfer fee ${s.baseToken.symbol} charges on every move of the token` : ""}, and the seat earns about $${r(earn.feesPerDayUsd, 2)} a day, so it pays that back in ${r(earn.paybackHours, 1)}h, past the ${env.maxPaybackHours}h the policy will wait. ${poolClause(o, hot)}.`,
