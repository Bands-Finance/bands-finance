/**
 * WHAT HE LEARNED, AS ONE SHAPE FOR THE PUBLIC SURFACES. The single view four surfaces render, so
 * they can never disagree: his observation (src/agent/observation.ts), GET /api/status
 * (src/server.ts), the site's web/public/learned.json (src/scripts/snapshot.ts) and the
 * `bands_lessons` MCP tool (src/platform/mcp/server.ts).
 *
 * Its sibling src/learn/view.ts is the ENGINE view: it reads the files and does the arithmetic for
 * `npm run learning` and the learner's own tests. This file holds TYPES ONLY, plus one empty
 * constructor. Nothing here reads a file, decides anything or imports anything: it is the contract
 * between the learner that writes DATA_DIR/learning.jsonl and the surfaces that read it back. The
 * learner (src/learn) owns the numbers; the surfaces own the words. readLearnedView() in
 * src/status.ts is the one builder of this shape.
 *
