...
import { forgetBand, knifeReason, moveAfterSec, outOfRangeSec, rangeOverWindowPct, recordPrice, trackOutOfRange } from "./engine/exit";
...
async function runPool(app: App, o: Observed, all: Observed[], sol: number): Promise<JournalEntry> {
  ...
  const stoppedOut = execution.ok && !!execution.closed && !rehearsal && (directive?.kind === "STOP" || verdict.overrides.some((v) => v.startsWith("stop-loss")));
  if (stoppedOut) {
